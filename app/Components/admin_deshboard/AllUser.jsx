...
// const AllUser = () => {
//     const baseUrl = 'https://smartstay-api.up.railway.app/auth/alluser';
...
const AllUser = () => {
    ...
    return (
        <div className="p-4 md:p-6 bg-white rounded-lg w-full">

            {/* Counters */}
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                <div className="bg-purple-100 text-purple-800 p-4 md:p-5 rounded-lg shadow-sm text-center">
                    <span className="text-3xl md:text-4xl font-bold">{adminCount}</span>
                    <div className="mt-1 font-medium text-sm md:text-base">Total Admins</div>
                </div>
                <div className="bg-yellow-100 text-yellow-800 p-4 md:p-5 rounded-lg shadow-sm text-center">
                    <span className="text-3xl md:text-4xl font-bold">{ownerCount}</span>
                    <div className="mt-1 font-medium text-sm md:text-base">Total Owners</div>
                </div>
                <div className="bg-green-100 text-green-800 p-4 md:p-5 rounded-lg shadow-sm text-center">
                    <span className="text-3xl md:text-4xl font-bold">{userCount}</span>
                    <div className="mt-1 font-medium text-sm md:text-base">Total Users</div>
                </div>
            </div>

            {/* Search + Filters */}
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-2 sm:gap-0">
                <h2 className="text-lg md:text-xl font-bold">All Customers ({count})</h2>
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                    <select
                        value={selectedOption}
                        onChange={(e) => setSelectedOption(e.target.value)}
                        className="p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-purple-500 w-full sm:w-auto"
                    >
                        <option value="username">Username</option>
                        <option value="email">Email</option>
                    </select>
            ...
            {/* User Table */}
            <div className="h-72 md:h-96 overflow-y-auto border-t border-b border-gray-200">
                {users.length > 0 ? (
                    <AllCustomers users={users} />
                ) : (
